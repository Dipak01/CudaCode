...
void logError(const char * errArg, const char * eMsg){
	...
	puts("     - segment = simple segment scan based approach");
...
void verify(MatrixInfo * mat, MatrixInfo * vec, MatrixInfo * res){
	printf("Verifying results\n");
	float *seqMat = (float *)malloc(sizeof(float) * mat->M);
	memset(seqMat, 0, sizeof(float) * mat->M);
	//add content to our matrix
	for (int i = 0; i < mat->nz; i++) {
		seqMat[mat->rIndex[i]] += mat->val[i] * vec->val[mat->cIndex[i]];
	}
	//compare GPU matrix vs CPU matrix
	for (int i = 0; i < mat->M; i++) {
		float seqres = seqMat[i];
		float myres = res->val[i];
		float err = myres - seqres;
		if (err < 0){
			err = -1 * err;
		}
